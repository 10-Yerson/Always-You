        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/80 backdrop-blur-sm" onClick={() => setSelectedLetter(null)}>
          <div className="relative w-full max-w-md max-h-[85vh] overflow-y-auto bg-white rounded-2xl shadow-xl" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="p-5 space-y-4">
              ...
              {mediaItems.length > 0 && (
                <div className="relative">
                  <div className="bg-gray-100 rounded-xl overflow-hidden flex items-center justify-center min-h-[200px]">
                    {mediaItems[currentMediaIndex]?.type === 'image' && <img src={mediaItems[currentMediaIndex].url} alt="Imagen" className="w-full object-contain max-h-64" />}
                    {mediaItems[currentMediaIndex]?.type === 'video' && <video controls className="w-full max-h-64"><source src={mediaItems[currentMediaIndex].url} /></video>}
                    {mediaItems[currentMediaIndex]?.type === 'audio' && <div className="p-6 text-center"><Music className="w-12 h-12 text-gray-400 mx-auto mb-3" /><audio controls className="w-full"><source src={mediaItems[currentMediaIndex].url} /></audio></div>}
                  </div>
                  {mediaItems.length > 1 && (
                    <>
              ...
              <div className="text-center pt-2"><p className="text-blue-500 text-xs italic">"Cada palabra escrita es un latido de mi corazón"</p></div>
            </div>
          </div>
        </div>
      )}
